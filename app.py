...
for symptoms, age in test_cases:
    ...
    print(f"Symptoms: {symptoms}, Age: {age} → Score: {score}, Label: {label}")

GUIDELINES_DB = {
    "breath": [
        "WHO – IITT: Respiratory distress guidance",
    ...
    "vomit": [
        "WHO – Gastrointestinal Guidelines",
        "Monitor for dehydration and electrolytes"
    ],
    "cough": [
        "WHO – Respiratory Infection Guidelines",
        "Assess for infection and supportive care"
    ],
    "headache": [
        "WHO – Neurological Assessment Guidelines",
...
for symptoms in test_symptoms_list:
    ...
    print(f"Symptoms: {symptoms} → Guidelines: {guides}\n")
...
def adjust_urgency_rules():
    ...
    for s, count in symptom_counts.items():
        if count >= 3 and s not in {"breath", "pain"}:
            print(f"⚠️ Learning: Promoting symptom '{s}' to HIGH risk due to repeated feedback")
...
with gr.Blocks() as demo:
    gr.Markdown("## 🏥 Medical Triage System (CPU-only, Explainable, Feedback-enabled)")
